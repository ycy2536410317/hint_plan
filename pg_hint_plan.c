...
static DpTreeShape dp_tree_shape = DP_DEFAULT;
...
static RelOptInfo *pg_hint_plan_join_search(PlannerInfo *root,
											int levels_needed,
											List *initial_rels);
static void pg_hint_plan_set_baserel_size_estimates(PlannerInfo *root,
													RelOptInfo *rel);
...
static int findRelIdByName(PlannerInfo *root, char *aliasname);
static Relids get_bms_of_relids(PlannerInfo *root, char *aliasname);
...
static set_baserel_size_estimates_hook_type prev_set_baserel_size_estimates_hook = NULL;
...
void _PG_init(void)
{
	...
	prev_set_baserel_size_estimates_hook = set_baserel_size_estimates_hook;
	set_baserel_size_estimates_hook = pg_hint_plan_set_baserel_size_estimates;
...
void _PG_fini(void)
{
	...
	set_baserel_size_estimates_hook = prev_set_baserel_size_estimates_hook;
			  ...
			  const char *str)
{
	...
	{
		hint_ereport(str,
					 ("%s hint requires at least one relations.",
					  hint->base.keyword));
		hint->base.state = HINT_STATE_ERROR;
		return str;
	}
...
set_config_int32_option(const char *name, int32 value, GucContext context)
{
	...
	return set_config_option_noerror(name, buf, context,
...
setup_scan_method_enforcement(ScanMethodHint *scanhint, HintState *state)
{
	...
	if (enforce_mask == ENABLE_SEQSCAN || enforce_mask == ENABLE_INDEXSCAN ||
		enforce_mask == ENABLE_BITMAPSCAN || enforce_mask == ENABLE_TIDSCAN || enforce_mask == (ENABLE_INDEXSCAN | ENABLE_INDEXONLYSCAN))
		mask = enforce_mask;
	else
													...
													RelOptInfo *rel)
{
	...
	for (i = 0; i < current_hint_state->num_hints[HINT_TYPE_ROWS]; i++)
	{
		...
		rows_hint->joinrelids = get_bms_of_relids(root, rows_hint->relnames[0]);
		/*
		 * Skip this rows_hint if it is invalid from the first or it
		 * doesn't target any join rels.
		 */
		if (!rows_hint->joinrelids ||
			rows_hint->base.state == HINT_STATE_ERROR || bms_num_members(rows_hint->joinrelids) > 1)
			continue;

		if (bms_is_member(rel->relid, rows_hint->joinrelids))
		{
			/*
			...
			foreach (l, current_hint_state->parent_index_infos)
			{
				...
				for (i = 0; i < info->ncolumns; i++)
				{
					...
					if (strcmp(p_attname, c_attname) != 0 ||
						...
						((p_info->indoption[i] & INDOPTION_DESC) != 0) != info->reverse_sort[i] ||
						((p_info->indoption[i] & INDOPTION_NULLS_FIRST) != 0) != info->nulls_first[i])
						break;
				}
...
static Relids get_bms_of_relids(PlannerInfo *root, char *aliasname)
{
	int relid;
	Relids relids = NULL;
	relid = findRelIdByName(root, aliasname);
	if (relid == -1)
	{
		ereport(ERROR,
				(errcode(ERRCODE_UNDEFINED_TABLE),
				 errmsg("relation \"%s\" does not exist", aliasname)));
	}
